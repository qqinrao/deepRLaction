...
def evaluate_accuracy(net, data_iter):  #@save
    ...
    if isinstance(net, torch.nn.Module):
        net.eval()  # 将模型设置为评估模式
    metric = Accumulator(2)  # 创建两个变量，分别为正确预测数、预测总数
    with torch.no_grad():
...
class Vocab:
    ...
    def __init__(self, tokens=[], min_freq=0, reserved_tokens=[]):
        ...
        # Add tokens that meet the minimum frequency requirement
...
def load_corpus_time_machine(max_tokens=-1):  #@save
    """返回时光机器数据集的词元索引列表和词表"""
    #ARGS：max_tokens：限制加载的词元数量，-1表示不限制。
...
def predict_ch8(prefix, num_preds, net, vocab, device):
    ...
    #将prefix中的第一个字符在 vocab 中的下标先放到 outputs 中
    get_input = lambda: torch.tensor([outputs[-1]], device=device).reshape((1, 1))
    #将 outputs 最后一个元素放到 get_input 中，作为这一次的输入
    for y in prefix[1:]:  # 预热期
    #排除掉已经放到 output 中的第一个词，剩下的开始一一遍历
        _, state = net(get_input(), state)
        #这里不需要储存预测的结果，因为我们这个是训练，已经有标准答案，我们需要的是这一次预测所计算到的隐藏状态
...
def train_epoch_ch8(net, train_iter, loss, updater, device, use_random_iter):
    """Args：net：网络; train_iter:训练数据集; loss:损失函数; updater:更新器; device:设备; use_random_iter:是否使用随机抽样"""
    """训练网络一个迭代周期（定义见第8章）"""
    ...
    for X, Y in train_iter:
        if state is None or use_random_iter:
            ...
            #因为在随机采样中，两个子序列间是没有位置信息的，在每一个 batch 中要初始化隐藏状态，去掉位置信息。
            ...
            if isinstance(net, nn.Module) and not isinstance(state, tuple):
                # state对于nn.GRU是个张量
                state.detach_()
            #detach的作用是保留 state 中的值，但对 state 中的梯度不保留。
            else:
